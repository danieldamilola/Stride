...
- **Legacy emoji:** four glyphs still ship. Downloads empty state uses a folder, Error uses a warning triangle, OneTab uses Unicode stars, the dark mode banner uses a moon. New code must use SVG.
- **Color:** icons inherit `TextSecondary`, brighten on hover, take `Accent` when selected or active.
...
- Menu: surface fill, 1px border, radius 8, padding 4, shadow blur 12 depth 4 at 25%. Items radius 4, padding `8,6`, gesture hints right aligned in `TextSecondary`, separators 1px `BorderBrush` with `4,4` margins.
...
Known defect: every page except Downloads defines the dark `--border` as a cyclic `var` reference with `333` appended, so borders vanish in dark mode. Five pages also carry a dead duplicate token block whose values reference themselves. Fix both when touching a page token block.
...
3. **No emoji as UI icons in new code.** Downloads folder, Error warning triangle, OneTab stars, and the dark mode banner moon are legacy. Replace them with SVG over time.
