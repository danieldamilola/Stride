...
public sealed class ExtensionManager
{
    ...
    public async Task<string?> EnsureUBlockDownloadedAsync()
    {
        try
        {
            ...
            var localZipPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Resources", "Extensions", $"uBlock0_{UBlockVersion}.zip");

            Trace.WriteLine($"ExtensionManager: checking local uBlock zip at {localZipPath}");

            if (!await VerifyTofuHashAsync(localZipPath))
            {
                return null;
            }
