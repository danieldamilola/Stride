...
export async function analyze(text, settings, url) {
  ...
  switch (provider) {
    case "tclens":
      return await analyzeWithTCLens(text, apiKey, url);
    case "anthropic":
...
async function analyzeWithTCLens(text, apiKey, url) {
  const response = await fetchWithRetry(PROVIDERS.tclens.baseUrl, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${apiKey}`,
...
async function getErrorDetails(response) {
  try {
    const data = await response.json();
    return data.error?.message || typeof data.error === 'string' ? data.error : (data.message || "");
  } catch {
    return "";
  }
}
