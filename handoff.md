...
### 2026-08-17 - R4 TabEngine decomposition (F1 remainder)

- **`Engine\WebViewFactory`** (new, ~200 lines) - owns the WebView2-specific knowledge: `InitializeAsync` (environment + browser arguments), `BuildBrowserArguments`, `CreateWebViewControl`, `ConfigureCoreWebView`, `NavigateInitialUrl`, `TryInitializeExtensions`. Exposes `Environment` + `BrowserProcessExited`; TabEngine subscribes the process-death handling.
- **`Engine\WebViewIpcBridge`** (new, ~210 lines) - owns the IPC surface: `Wire` (message reception with per-session token verification, new-window→tab, fullscreen, window-close, download/dialog wiring) and `PostMessageToActiveTab`. Tab lifecycle callbacks (`isTabAlive`, `getWebView`, `createTab`, `activateAsync`, `closeTab`) are injected delegates - the bridge is engine-agnostic and unit-testable.
- TabEngine keeps tab lifecycle, per-tab event wiring, theming, teardown: **1162 → 905 lines**. Consumers unchanged (`engine.WebMessageReceived`/`FullScreenChanged`/`PostMessageToActiveTab` still public, now forwarded to the bridge).
- 39/39 tests passing; 0 build errors.
